...
/**
 * UUID sanity check (because your Supabase ids are uuid)
 */
function isUuid(v) {
...
/**
 * Lazy-init Supabase so /health always works (prevents Cloud Run confusion)
 */
let _supabase = null;
 ...
 * persists latest access token back to Supabase.
 */
async function getGa4AuthForWorkspace(workspace_id) {
  if (!workspace_id || !isUuid(workspace_id)) {
    throw new Error("workspace_id missing/invalid");
  }
  ...
  if (c.access_token && c.expiry_date) {
    await supabase
      .from("oauth_tokens")
      .update({ access_token: c.access_token, expiry_date: c.expiry_date, updated_at: new Date().toISOString() })
      .eq("client_id", workspace_id)
      .eq("connector_type", "ga4");
  }
...
async function getBoundGa4Property(workspace_id) {
  ...
  return data; // external_id = "properties/..."
}
...
function buildGa4Request(preset, date_from, date_to, limit = 50, offset = 0) {
  const base = { dateRanges: [{ startDate: date_from, endDate: date_to }], limit, offset };
...
/**
 * META cache for fields endpoint
 */
const _ga4MetaCache = new Map(); // key: propertyId
...
async function getGa4FieldsForWorkspace(workspace_id) {
  ...
  const metaResp = await dataApi.properties.getMetadata({ name: `${propertyId}/metadata` });
...
app.get("/auth/ga4/start", async (req, res) => {
  try {
    ...
    if (missing.length) {
      return res.status(500).json({ error: "Server misconfigured (missing env vars)", missing });
    }

    const nonce = crypto.randomUUID();
    const state = signState({ workspace_id, nonce });

    return res.redirect(getAuthUrl(state));
  } catch (e) {
...
app.get("/auth/ga4/callback", async (req, res) => {
  ...
  try {
    ...
    const { error } = await supabase.from("oauth_tokens").upsert(payload, {
      onConflict: "client_id,connector_type",
    });
    if (error) throw error;
 ...
 * GET /ga4/properties?workspace_id=...
 */
...
app.post("/ga4/bind", async (req, res) => {
  ...
  try {
    ...
    if (!property_id) return res.status(400).json({ error: "property_id required (example: properties/523...)" });

    // Validate property is accessible
    const auth = await getGa4AuthForWorkspace(workspace_id);
    const admin = google.analyticsadmin({ version: "v1beta", auth });
    const resp = await admin.accountSummaries.list();
...
app.get("/ga4/report", async (req, res) => {
  try {
    ...
    const date_from =
      (Array.isArray(req.query.date_from) ? req.query.date_from[0] : req.query.date_from) || "30daysAgo";
    ...
    const resp = await dataApi.properties.runReport({ property: binding.external_id, requestBody });
    ...
    const envelope = {
      ...
      request: { preset, date_from, date_to, property_id: binding.external_id, limit, offset },
      data: normalized,
...
app.post("/ga4/store", async (req, res) => {
  ...
  try {
    ...
    const preset = body.preset || "overview";
    const date_from = body.date_from || "30daysAgo";
    const date_to = body.date_to || "today";
    const limit = Math.max(1, Math.min(500, Number(body.limit ?? 50)));
    const offset = Math.max(0, Number(body.offset ?? 0));

    if (!workspace_id) return res.status(400).json({ error: "workspace_id required" });
    if (!isUuid(workspace_id)) return res.status(400).json({ error: "workspace_id must be uuid" });
    ...
    return res.json({ ok: true, stored: data, dedupe_key });
  } catch (e) {
    await logAudit({
      workspace_id,
      action: "preset_store",
    ...
    return res.status(500).json({ error: "Failed to store GA4 preset", details: String(e?.message || e) });
  }
});
...
app.post("/ga4/store-presets", async (req, res) => {
  ...
  try {
    ...
    const presets = Array.isArray(body.presets)
      ? body.presets
      : ["overview", "top_pages", "acquisition", "geo", "devices", "events", "landing_pages", "conversions"];
    ...
    for (const preset of presets) {
      try {
        const requestBody = buildGa4Request(String(preset), date_from, date_to, limit, offset);
        const resp = await dataApi.properties.runReport({ property: binding.external_id, requestBody });
        const normalized = normalizeReport(resp.data);
        ...
        results.push({ preset, ok: false, error: String(innerErr?.message || innerErr) });
      }
    }
    ...
    console.error("❌ store-presets error:", e);
...
app.post("/ga4/custom-report", async (req, res) => {
  try {
    ...
    if (errors.length) {
      return res.status(400).json({ error: "Invalid fields", errors, hint: "Use GET /ga4/fields" });
    }
...
app.post("/ga4/custom-store", async (req, res) => {
  ...
  try {
    ...
    // ✅ Always inserts workspace_id (never null)
    const { data, error } = await supabase
      .from("connector_snapshots")
      .upsert(
        {
          ...
          date_from,
          date_to,
          dimensions: stableJoin(dimensions) || null,
          metrics: stableJoin(metrics) || null,
          limit_value: limit,
          offset_value: offset,
          dedupe_key,
          payload_json: envelope,
...
/**
 * Latest snapshot (NO Google call)
 * GET /ga4/latest?workspace_id=...&preset=overview&date_from=30daysAgo&date_to=today&limit=50&offset=0
 *
 * or custom:
 * GET /ga4/latest?workspace_id=...&date_from=30daysAgo&date_to=today&dimensions=country&metrics=activeUsers,sessions&limit=50&offset=0
 */
app.get("/ga4/latest", async (req, res) => {
  try {
    ...
    if (preset) {
      ...
      const dimsRaw = Array.isArray(req.query.dimensions) ? req.query.dimensions[0] : req.query.dimensions;
      const metsRaw = Array.isArray(req.query.metrics) ? req.query.metrics[0] : req.query.metrics;

      const dims = dimsRaw ? String(dimsRaw).split(",").filter(Boolean) : [];
      const mets = metsRaw ? String(metsRaw).split(",").filter(Boolean) : [];
    ...
    if (error || !data) {
      return res.status(404).json({ error: "No snapshot found", dedupe_key });
    }

    return res.json({
      ok: true,
      dedupe_key,
      pulled_at: data.pulled_at || data.updated_at || null,
      updated_at: data.updated_at || null,
      row_count: data.row_count ?? data.payload_json?.quality?.row_count ?? 0,
      payload: data.payload_json || null,
    });
  } catch (e) {
    console.error("❌ latest error:", e);
    return res.status(500).json({ error: "Failed to fetch latest snapshot", details: String(e?.message || e) });
  }
});
...
process.on("uncaughtException", (err) => console.error("uncaughtException:", err));import "dotenv/config";
import express from "express";
import crypto from "crypto";
import { createClient } from "@supabase/supabase-js";
import { google } from "googleapis";
